...
export function middleware(request: NextRequest) {
  ...
  if(!isPublicRoute){
    if(role === UserRole.ADMIN){
      const allowedPaths = ["/master", "/stations", "/shows", "/users","/media-houses","/promotions","/finance","/commission","/reports","/settings","/transactions","/stations"];
      ...
      return NextResponse.redirect(new URL(`/stations/${decodedToken.station}/dashboard`, request.url));
      }
