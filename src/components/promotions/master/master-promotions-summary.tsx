...
function MasterPromotionsSummary() {
  const dispatch = useDispatch<AppDispatch>();
  const loading = useSelector((state: RootState) => state.promotions.loading);
  const promotions = useSelector(
    (state: RootState) => state.promotions.masterPromotions
  );

  useEffect(() => {
    dispatch(getMasterPromotions());
  }, [dispatch]);
  return (
    <div className="mt-4">
      {loading === "pending" ? (
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {promotions.slice(0,3).map((promo) => (
              <div
                key={promo._id}
                className="bg-gray-900 rounded-lg shadow-lg p-4 border-l-4 border-green-500"
              >
...
export default MasterPromotionsSummary;
