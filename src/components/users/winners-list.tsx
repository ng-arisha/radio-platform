...
import { getStationWinners } from "@/lib/users/users";
...
function WinnersList() {
    ...
    const winners = useSelector((state: RootState) => state.users.winners)
    const loading = useSelector((state: RootState) => state.users.loadingWinners)
    const params = useParams<{ stationId: string }>();

    useEffect(() => {
        dispatch(getStationWinners({stationId:params.stationId}))
    }, [params.stationId])
    return (
        <div>
           {
