...
import { getShowWinners } from "@/lib/users/users";
...
function ShowWinnersList() {
    ...
    const winners = useSelector((state: RootState) => state.users.showWinners)
    const loading = useSelector((state: RootState) => state.users.loadingWinners)
    const params = useParams<{ showId: string }>();

    useEffect(() => {
            dispatch(getShowWinners({showId:params.showId}))
        }, [params.showId])
    return (
        <div>
        {
...
export default ShowWinnersList
