...
import { withdraw } from "@/lib/wallet/wallet";
import { formatCurrency } from "@/utils/utils";
import { Download, SunIcon, User } from "lucide-react";
...
function ProfileSection() {
  ...
  const profileData = useSelector(
    (state: RootState) => state.users.profileData
  );
  const withdrawing = useSelector((state: RootState) => state.wallet.loading);
  ...
  const handleWithdrawal = async () => {
    ...
    const res = await dispatch(withdraw(data));
          ...
          <div className="w-full max-w-3xl bg-gray-800 rounded-lg p-6 border border-gray-700">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6">
              ...
              <TextColumn
                ...
                value={formatCurrency(profileData.availableBalance)}
              />
              ...
              <TextColumn
                label="Active days"
                value={profileData.activedays.join(",")}
              />
              <TextColumn
