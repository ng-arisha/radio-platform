...
import { getStationCommission } from "@/lib/commission/commission";
...
import AssignStationCommissionRate from "./assign-station-commission-rate";


function StationCommission() {
    const range = [
        {
          label: "1 hr Ago",
          value: "1h",
  ...
  const masterCommission = useSelector(
    (state: RootState) => state.commission.stationCommission
  );
  const params = useParams<{ stationId: string }>();

  const fetchMasterCommission = async (range: string,fromDate:string,endDate:string) => {
    await dispatch(
        getStationCommission({ range: range, id: params.stationId, startDate: fromDate, endDate: endDate })
    );
  };
    ...
    return (
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-medium text-white">
           Station Commission Dashboard
          </h1>
        </div>
        <p className="text-gray-400">
          Overview of Revenue performance accross all shows
        </p>
      </div>
      ...
      {loading === "pending" || masterCommission === null ? (
        ...
        <div>
          ...
          {masterCommission.shows.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-600">
                      {masterCommission.shows.map((item) => (
                        <tr
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-gray-300">
                            ...
                            {masterCommission.stationName}-{item.showName}
                            </div>
                          </td>
                          <td className="px-6 py-4 text-gray-300">
                          ...
                          <td className="px-6 py-4">
                            <AssignStationCommissionRate item={item} />
                          </td>
                        </tr>
                      ))}
                    </tbody>
...
export default StationCommission
