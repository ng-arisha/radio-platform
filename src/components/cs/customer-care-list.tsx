...
import { getStationCustomerCare } from "@/lib/users/users";
import { Radio, SunIcon } from "lucide-react";
import { useParams } from "next/navigation";
import { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";

function CustomerCareList() {
    const dispatch = useDispatch<AppDispatch>();
  const loading = useSelector((state: RootState) => state.stations.loading);
  const presenters = useSelector(
    (state: RootState) => state.users.customerCareUsers
  );
  const params = useParams<{ stationId: string }>();

  useEffect(() => {
    dispatch(getStationCustomerCare({ id: params.stationId }));
